...
static int loadOBJ(meshObject *object, char *filename){
	if (!openOBJ_file(filename))
		return 0;
	createVertexArrayOBJ(&(object->vertexArray), &(object->vertexCount));
	createElementArrayOBJ(&(object->elementArray), &(object->elementCount));
	if(!readOBG_file())
		return 0;
...
static void genBuffers(meshObject *object){
	...
	//glBufferData(GL_ARRAY_BUFFER, sizeof(object->vertexColors), object->vertexColors, GL_STATIC_DRAW);
