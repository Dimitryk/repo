...
long createVertexArray(GLfloat **vArray) {
	if (ply==NULL)
		return 0;
	if(vArray==NULL){
		*vArray = (GLfloat*) malloc(nvertices * sizeof(GLfloat));
	}else{
		*vArray = (GLfloat*) realloc(*vArray,nvertices * sizeof(GLfloat));
	}
	vertexArray = *vArray;
...
long createTriList(GLuint **tArray){
	if (ply==NULL)
		return 0;
	if(tArray==NULL){
		*tArray = (GLuint*) malloc(ntriangles *  sizeof(GLuint));
	}else{
		*tArray = (GLuint*) realloc(*tArray,ntriangles * sizeof(GLuint));
	}
	triArray = *tArray;
